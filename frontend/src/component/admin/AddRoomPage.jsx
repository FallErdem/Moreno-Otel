...
const AddRoomPage = () => {
    ...
    const addRoom = async () => {
        if (!roomDetails.roomType || !roomDetails.roomPrice || !roomDetails.roomDescription) {
            setError('Tüm oda detayları sağlanmalıdır..');
            setTimeout(() => setError(''), 5000);
            return;
        }

        if (!window.confirm('Bu odayı eklemek istediğinizden emin misiniz?')) {
            return
        }
            ...
            <div className="edit-room-form">
                ...
                <div className="form-group">
                    ...
                    <select value={roomDetails.roomType} onChange={handleRoomTypeChange}>
                        ...
                        <option value="new">Diğer (lütfen belirtiniz)</option>
                    </select>
                    {newRoomType && (
                        <input
                            type="text"
                            name="roomType"
                            placeholder="Yeni oda tipi girin"
                ...
                <button className="update-button" onClick={addRoom}>Oda Ekle</button>
            </div>
        </div>
    );
};
