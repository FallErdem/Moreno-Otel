...
const RoomDetailsPage = () => {
  ...
  const handleConfirmBooking = async () => {
    ...
    if (isNaN(numAdults) || numAdults < 1 || isNaN(numChildren) || numChildren < 0) {
      setErrorMessage('Lütfen yetişkinler ve çocuklar için geçerli sayılar girin.');
      setTimeout(() => setErrorMessage(''), 5000); // Clear error message after 5 seconds
      return;
    }
    ...
    <div className="room-details-booking">
      {showMessage && (
        <p className="booking-success-message">
          Rezervasyon başarılı! Onay kodu: {confirmationCode}. Rezervasyon detaylarınızın bir SMS ve e-posta bildirimi size gönderilmiştir.
        </p>
      )}
      {errorMessage && (
      ...
      <div className="room-details-info">
        <h3>{roomType}</h3>
        <p>Fiyat: ${roomPrice} / gece</p>
        <p>{description}</p>
      </div>
      {bookings && bookings.length > 0 && (
        <div>
          <h3>Mevcut Rezervasyon Detayları</h3>
          <ul className="booking-list">
            {bookings.map((booking, index) => (
              <li key={booking.id} className="booking-item">
                <span className="booking-number">Rezervasyon {index + 1} </span>
                <span className="booking-text">Giriş: {booking.checkInDate} </span>
                <span className="booking-text">Çıkış: {booking.checkOutDate}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="booking-info">
        <button className="book-now-button" onClick={() => setShowDatePicker(true)}>Hemen Rezervasyon Yap</button>
        ...
        {showDatePicker && (
          <div className="date-picker-container">
            ...
            <div className='guest-container'>
              ...
              <button className="confirm-booking" onClick={handleConfirmBooking}>Onayla</button>
            </div>
          </div>
        )}
        {totalPrice > 0 && (
          <div className="total-price">
            ...
            <p>Toplam Misafir: {totalGuests}</p>
